...
# Install to ~/.copilot/hooks/ when running inside tmux.
...
curl -s --max-time 2 -X POST http://localhost:7777/notify \
  ...
  -d "{\"sessionId\":\"$SESSION_ID\",\"message\":\"$MESSAGE\",\"hookType\":\"$HOOK_TYPE\",\"project\":\"$PROJECT\",\"tmuxTarget\":\"$TMUX_TARGET\",\"agent\":\"copilot\",\"client\":\"$CLIENT\"}" \
  > /dev/null || true
